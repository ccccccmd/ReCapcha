    ...
    public class ImageProcess
    {
        ...
        public static int GetDgGrayValue(Image img)
        {
            ...
            for (k = 0; k <= 255; k++)
            {
                ...
                pixelNum[k] = (int) (total / 5.0 + 0.5); // pixelNum[k] 的灰度值是前后5个点的平均值
            }
            ...
            unsafe
            {
                ...
                for (int y = 0; y < h; y++)
                {
                    for (int x = 0; x < w; x++)
                    {
                        ...
                        b = p[0];
                        pOut[0] = pOut[1] =
                            pOut[2] = (byte) ((byte) (0.2125 * r + 0.7154 * g + 0.0721 * b) >= v ? 255 : 0);
        ...
        /// <returns></returns>
        public static List<Bitmap> PreProcess(Image img, int maxNearPoints, int smallPicWidth, int smallPicHeight,
            int type = 2, bool is4Chars = true, int charWidth = 10)
        {
        ...
        /// <param name="smallPicHeight">切分小图高度</param>
        /// <paramname="charWidht">
        ///     验证码字符宽度，细节处理时用到</param>
        ///     <returns>识别后的验证码字符</returns>
        public static string GetCAPTCHACode(Image img, string zimoPath, int maxNearPoints, int smallPicWidth,
            int smallPicHeight, int type = 2, bool is4Chars = true, int charWidth = 10)
        {
            //1.0验证码前期处理
            List<Bitmap> list = PreProcess(img, maxNearPoints, smallPicWidth, smallPicHeight, type, is4Chars,
                charWidth);
            string yanzhengma = "";
            //2.0识别
            if (list.Count > 0)
            {
                ...
                for (int i = 0; i < list.Count; i++)
                {
                    ...
                    {
                        string[] subZimo = zimo[j].Split(new[] {"--"}, StringSplitOptions.None);
                        //2.3计算相似度
        ...
        /// <returns>识别后的验证码字符</returns>
        public static string GetCAPTCHACode(Image img, string zimoPath, int charNum)
        {
            return GetCAPTCHACode(img, zimoPath, 1, img.Width / charNum, img.Height);
        }
        ...
        public static void WriteZimo(Bitmap smallPic, string zimoPath, string CAPTCHACode)
        {
        ...
        public static string Array2String(int[] array, bool flag)
        {
            ...
            if (flag)
            {
                ...
                {
                    int eIndex = str.LastIndexOf("0", StringComparison.Ordinal);
        ...
        public static Point GetYBeyond(int[,] input)
        {
            ...
            for (int i = 0; i < h; i++)
            {
                ...
                if (str.IndexOf("1", StringComparison.Ordinal) > -1)
                {
                    y1 = i;
                    break;
                }
